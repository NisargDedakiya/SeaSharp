...
| Domains live | Identity/Orgs, Trade Intelligence, RFQ Marketplace, Logistics (stub), Trade Finance (request/approve/fund MVP — exporter + importer financing requests, investor funding, no repayment tracking yet), STS (exporter + importer), KYC (stub), event log + in-app notifications | All of the above, plus Activity Center UI, Company Profiles, Admin Console, Wallet/Ledger, full AI service layer, email/SMS delivery |
| Dashboard | **Widget-based, per-profile-per-org configurable layout** (`dashboard_layouts` table, `PATCH /api/dashboard/layout`) — STS, KYC, Loan, RFQs, Shipments, Revenue, Notifications, Investments (investor browse/fund) widgets render real data; Calendar/Tasks are explicit "coming soon" placeholders (no calendar/task domain exists yet) ✅ | Same, plus Calendar/Tasks backed by real domains once they exist |
